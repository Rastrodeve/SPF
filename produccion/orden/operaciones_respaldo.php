...
// Opcion 2 -- Funcion que Crea la Vista Previa de la orden de produccion
    function f_vista_previa($dbConn){
        ...
        $consulta="SELECT p.gprId,p.gprComprobante,p.gprCantidad, p.gprProcesado, p.gprestadoDetalle, p.gprTurno,
        ...
        WHERE p.espId = e.espId AND c.cliId = p.cliId AND p.gprCantidad <> p.gprProcesado AND p.gprEstado = 1 AND p.espId = :id
        ...
        while ($row = $sql->fetch()) {
            ...
            if (count($array)==3) {
                if (is_numeric($array[0]) && is_numeric($array[1]) && is_numeric($array[2])) {
                    ...
                    if ($bandera==true) {
                        $saldo = intval($row["gprCantidad"]) -  intval($row["gprProcesado"]);
                        if ($saldo > 0) {
							...
							$total_corralaje = 0;
                            $Corralaje = '<td >'.$TiempoEstancia.'</td> <td class="table-info text-right"><b>DESACTIVADO</b></td>';
                            if ($row["espEstadoCorralaje"] == 1 ) {//1 El corralaje esta activado
                                ...
                                $Corralaje = '<td >'.$TiempoEstancia.'</td> <td class="table-info text-right"><b>$ '.number_format($total_corralaje, 2).'</b></td>';
							}
                            ...
                            $Resultado .='<tr>
								...
								'.$Corralaje.'
								<td class="table-danger text-right"><b>$ '.number_format($totalPagar, 2).' </b></td>
							</tr>';
                        }
                    }
                } else{
		...
		if ($cont > 0) {//Muetsra el footer y el boton si el contador es mayor a 0
			...
			$CorralTh='<th class=" table-info text-right" colspan="2" >$ '.number_format($SubtotalCorralaje, 2).'</th>';//Se declara el corralaje como No dispobile
			$footer = '<tfoot>
				<tr>
					<th colspan="2" >TOTALES</th>
					<th class="table-warning">'.$CantidaGlobal.'</th>
					'.$FtotalesServicios.'
					<th class="text-right table-success">$ '.number_format($Subtotal1Global, 2).'</th>
					'.$CorralTh.'
					<th class="text-right table-danger">$ '.number_format($TotalPagarGlobal, 2).'</th>
				</tr>
			</tfoot>';
		}
		$titulo = '<h5 class="mb-2 text-center" style="cursor:pointer;" onclick="recargar_vista_previa()">
							<b>'.strtoupper($Ganado).'</b> TOTAL A PROCESAR:  <b style="font-size:23px;">'.$CantidaGlobal	.'</b>
							</h5>'; 
	...
	function DataEncabezado($arrayServicios){
		...
		return '<thead>
			<tr style="text-align:center;">
				<th colspan="3" class="table-warning">DATOS GENERALES</th>
				<th colspan="'.($Total_Servicios + 1).'" class="table-success">SERVICIOS</th>
				<th colspan="2" class="table-info">CORRALAJE</th>
				<th rowspan="2" class="table-danger" style="max-width:60px;">TOTAL</th>
			</tr>
			<tr>
				<th class="table-warning" style="max-width:20px;">#</th>
				<th class="table-warning" style="min-width:200px;">CLIENTE</th>
				<th class="table-warning" style="max-width:35px;">CANT.</th>
				'.$Servicios.'
				<th class="table-success" style="max-width:70px;">Subtotal 1</th>
				<th style="max-width:100px;">Tiempo de Estancia</th>
				<th class="table-info" style="max-width:70px;">Subtotal 2</th>
			</tr>
		</thead>';
	}
...
// "Funcion 7" Retorna el tipo de espcie dependiendo el codigo del animal
        ...
        for ($i=0; $i < count($ArrayDatosOrden); $i++) {
            ...
            if ($IdOrden != false) {
                ...
                $ArrayCabecera = array(
                    ...
                    $ArrayDatosOrden[$i][15],// Detalle1 = Nombre Cliente 
                    $ArrayDatosOrden[$i][16],// Detalle2 = Dirección Cliente
                    ...
                    0,// Valor4 = 0 Pago de Contado (Se asume cobro en efectivo) 1 Crédito
                    $IdOrden, // Id de la orden de produccion
                ...
                if (InsertarFacturaYupakLocal($dbConn,$ArrayCabecera)){
                    if (InstertarSQLYupak($ArrayCabecera)){
                        if(!UpdateTableGuiaProceso($dbConn,$ArrayDatosOrden[$i][11]))return 'YUPAK - No se pudo actualizar la guia para el cliente '.$ArrayDatosOrden[$i][15]; 
...
// "F Insterar 1" Esta funcion crea el numero que llevara la orden de produccion
	function Numero_Ordinal($dbConn,$Tipo){
        ...
        WHERE o.espId = e.espId AND e.espId = :id AND o.ordFecha AND o.ordTipo = 0 
		...
		while($row = $sql->fetch()) {
            ...
            if (count($Array1)==2) {
                ...
                if ($letraBuscar==$row["espLetra"]) {
                    ...
                    return "EMRAQ-".$Juliano.$row["espLetra"].$resultado.$numero;
                }
            }
		}
        ...
        $resultado="";
        for ($i=$cantidad; $i < $maximo; $i++) $resultado .= "0";
		return "EMRAQ-".$Juliano.$letra.$resultado.$numero;
	}
...
// "F Insertar 2" Esta funcion crea el numero de documento Yupak
	function Numero_DocumentoYupack($Codigo_Caja){
		...
		if($row = mssql_fetch_array($sql2)){
            ...
            $new_Num_Documento = $Codigo_Caja."000000001";
		}
		return "00".$new_Num_Documento;
	}

// "F Insertar 5" Funcion de insertar la orden de producción
	function InsertarOrdenTabla($dbConn,$Array,$doccumentoYupak,$especie){
		try {
			$consulta="INSERT INTO tbl_p_orden(ordNumOrden,ordFecha,ordCantidad,ordProcesado,ordTiempoEstancia,ordEstanciaPermitida,ordEstadoCorralaje,ordEstanciaCorralaje,ordTiempoCobrar,ordPrecioCorralaje,ordTotalCorralaje,Num_Documento,gprId,espId)
			VALUES (:ordNumOrden,:ordFecha,:ordCantidad,:ordProcesado,:ordTiempoEstancia,:ordEstanciaPermitida,:ordEstadoCorralaje,:ordEstanciaCorralaje,:ordTiempoCobrar,:ordPrecioCorralaje,:ordTotalCorralaje,:Num_Documento,:gprId,:espId)";
				$sql= $dbConn->prepare($consulta);
...
// "Otras funciones 8" Esta funcion actualiza el procesado
	function UpdateTableGuiaProceso($dbConn,$Id){
        try {
...
// "Otras funciones 9" Esta funcion calcula el Numero Juliano
	...
	function UpdateTableGuiaDetalle($dbConn,$Orden,$Id){
		...
		while($row = $sql->fetch()) {
			if ($row["dtProceso"]!=0) {
                if (Comprobar_Orden_Guia($dbConn,$row["dtId"])==0) {
    ...
    function obtner_cantidad_detallle($dbConn,$Id){
        $consulta="SELECT * FROM tbl_detalle_guia_new WHERE id_contador = :id  AND  ordId is NULL ";
		$sql= $dbConn->prepare($consulta);
		$sql->bindValue(':id',$Id);
		$sql->execute();
        $cont = 0;
...
// "Otras funciones 8" Esta funcion comprueba si existen alguna guia emergente con
function Comprobar_Orden_Guia($dbConn,$Id){
    $cont=0;
    $consulta="SELECT * FROM tbl_orden_emergente_new WHERE dtId=:id";
    $sql= $dbConn->prepare($consulta);
    $sql->bindValue(':id',$Id);
    $sql->execute();
    while($row = $sql->fetch()) {
