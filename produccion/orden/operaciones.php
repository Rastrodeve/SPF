...
if (isset($_REQUEST["op"])) {
    ...
    // echo Numero_DocumentoYupack(1009);
}
...
function f_vista_previa($dbConn)
{
    ...
    $consulta = "SELECT p.gprId,p.gprComprobante,(p.gprMacho + p.gprHembra) AS gprCantidad, p.gprestadoDetalle, p.gprTurno,
                    ...
                    p.gprEstadoPagado, p.gprHabilitado
        FROM tbl_r_guiaproceso p, tbl_a_clientes c, tbl_a_especies e 
        WHERE p.espId = e.espId AND c.cliId = p.cliId AND p.gprEstado = 1 AND p.espId = :id AND p.gprEliminado = 0 AND p.gprHabilitado = 1
        ...
        if (count($array) == 3) {
            if (is_numeric($array[0]) && is_numeric($array[1]) && is_numeric($array[2])) {
                ...
                if ($bandera == true) {
                    ...
                    // print_r($cant[0]);
                    $saldo = (intval($cant[0]) + intval($cant[1]) + intval($cant[2]) + intval($cant[3]) + intval($cant[4])) -  $procesado;
                    // $saldo = (intval($cant[0])) -  $procesado;
                    if ($saldo > 0) {
...
function f_tiempo_minimo($FechaIngreso, $estado, $FechaPermitida)
{
    ...
    if ($ArrayTiempoEstancia[0] > $ArrayFechaPermitida[0]) { //Si el tiempo de estancia es mayor al tiempo permitido se muestra la guia
        ...
        $numero = date("N");
        $fin = "08:30:00"; // Los dias impares acaban el dia a las 11:30
...
function f_obtener_procesar($dbConn, $Id)
{
    ...
    $consulta = "SELECT faeCantidad FROM tbl_p_faenamiento f, tbl_p_orden o
            WHERE  f.ordId = o.ordId AND o.ordTipo = 0 AND o.gprId = :id";
    $sql = $dbConn->prepare($consulta);
    $sql->bindValue(':id', $Id);
    $sql->execute();
    while ($row = $sql->fetch()) {
        ...
        for ($i = 0; $i < count($ArrayDatosOrden); $i++) {
            ...
            if ($IdOrden != false) {
                ...
                $ArrayCabecera = array(
                    ...
                    utf8_decode($ArrayDatosOrden[$i][15]), // Detalle1 = Nombre Cliente 
                    utf8_decode($ArrayDatosOrden[$i][16]), // Detalle2 = Dirección Cliente
...
function Numero_Ordinal($dbConn, $Tipo)
{
    ...
    $consulta = "SELECT o.ordNumOrden,e.espLetra 
        ...
        WHERE o.espId = e.espId AND e.espId = :id  AND o.ordTipo = 0 AND o.ordFecha
...
{
    try {
        // $aaray = f_obtener_cantidades_orden($dbConn,$Id);
        // $total_dictamenes = $aaray[0] + $aaray[1] +$aaray[2];
        // if ($total_dictamenes == $total) {
...
function InstertarSQLYupak($Array)
{
    try {
        ...
        include '../../FilePHP/consql.php';
        $campos = " Yp_Empresa, Yp_Sucursal, Tipo, Documento, Caja, Num_Documento, Codigo_Id,
